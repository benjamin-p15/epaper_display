...
def render(force=False):
    ...
    if force or (now - _last_update >= 30 * 60): 
        ...
        for idx, assignment in enumerate(all_assignments[:max_assignments]):
            ...
            screen.add_rectangle(position=(course_box_x, y - 0.02), size=(course_box_w, y_gap - 0.01), fill=1, radius=6, thickness=2)
            screen.add_rectangle(position=(assignment_box_x, y - 0.02), size=(assignment_box_w, y_gap - 0.01), fill=1, radius=6, thickness=2)
            screen.add_rectangle(position=(due_box_x, y - 0.02), size=(due_box_w, y_gap - 0.01), fill=1, radius=6, thickness=2)
