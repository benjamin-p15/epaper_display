...
def render():
    ...
    if now - _last_update >= 5 * 60:
        ...
        for i in range(7):
            ...
            screen.add_text([{"text": hours[i], "size": 16}], position=(0+i*0.142 + 0.0675, 0.745),bold=True)
        for i, hour in enumerate(forecast_data["hourly"][:6]):
            icon = weather_to_icon(hour.get("weather"))
            screen.add_image(os.path.join(script_directory, "icons", icon),(0+i*0.142 + 0.047, 0.80),(0.07, 0.1),invert=True,color_black=True)
