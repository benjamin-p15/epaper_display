...
def render():
    ...
    if now - _last_update >= 5 * 60:
        ...
        screen.add_image(os.path.join(script_directory, "icons", "sunrise.png"),(0.6, 0.28),(0.05,0.05),invert=True)
        screen.add_text([
            {"text": f"{sunrise_time}", "size": 36},
            {"text": f"{sunrise_period}", "size": 18, "align": "bottom"}
        ], position=(0.65, 0.25),align="left")
        screen.add_image(os.path.join(script_directory, "icons", "sunset.png"),(0.6, 0.38),(0.05,0.05),invert=True)
        screen.add_text([
        ...
        # Wind speed
        data=[]
        screen.add_image(os.path.join(script_directory, "icons", "wind.png"),(0.39, 0.58),(0.05,0.05),invert=True)
        try:
