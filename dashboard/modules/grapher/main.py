...
def draw_grid(draw, width, height, scale_x, scale_y, graph_scale):
    # Draw vertical lines
    for i in range(0, width + 1, scale_x):
        color = 0 if i == width // 2 else 128
        draw.line((i, 0, i, height), fill=color)

        # Calculate number for axis
        num = (i - width // 2) // scale_x
        if num % graph_scale == 0:
            text_x = min(max(i + 2, 0), width - 15)  # clamp inside image
            text_y = height // 2 + 2
            draw.text((text_x, text_y), str(num), fill=0)

    # Draw horizontal lines
    for j in range(0, height + 1, scale_y):
        ...
        if num % graph_scale == 0:
            text_x = width // 2 + 2
            text_y = min(max(j + 2, 0), height - 10)  # clamp inside image
            draw.text((text_x, text_y), str(num), fill=0)
